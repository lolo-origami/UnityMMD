...
*/
        ...
        /*Pass
        {
            ...
            Stencil
            {
                Ref  [_StencilOverrideRef]     // Projector が書いた値
                Comp Equal                     // 影が落ちる部分だけ通す
                Pass Keep
            }

            ZWrite Off                         // 再描画なので書かない
            ZTest LEqual
            Cull[_Cull]
            ...
            {
                // ベース色をもう一度取得
                half3 baseCol = SAMPLE_TEXTURE2D(_Main, sampler_FaceTex, i.uv).rgb;

                // ブレンド
                half3 finalCol = lerp(baseCol, _StencilShadowColor.rgb, _StencilShadowIntencity);

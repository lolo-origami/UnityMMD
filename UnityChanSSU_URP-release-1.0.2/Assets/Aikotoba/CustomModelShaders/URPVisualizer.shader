Shader "Universal Render Pipeline/URPVisualizer"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass{
			...
			struct v2f
	        {
				...
				float4 windowSpacePos : TEXCOORD3;
			...
			v2f vert (appdata v)
	        {
				...
				o.windowSpacePos = ComputeScreenPos(o.vertex);
	        ...
	        float4 frag (v2f i) : SV_Target
	        {
        		...
        		float3 center = i.posWorld - _Center;
				float trails = Rings(center);
        		finalColor.rgb += trails * (0.5 + _Spectra) * _RingEmissionColor;

	        	//グリッドエフェクト
	        	float grid_d = HexGrid(center);
				float grid = grid_d > 0.0 ? 1.0 : 0.0;
	        	float circle = Circle(center);
	        	finalColor.rgb += _GridColor * grid * 0.01 + _GridColor * (grid * circle);
	        	...
	        	float2 coord = (i.windowSpacePos.xy / i.windowSpacePos.w);
				coord += n.xz * (g>0.0 && g<1.0 ? 1.0 : 0.0) * 0.02;
	        	for(int i=0; i<9; ++i)
	        	{
	        		finalColor.rgb += tex2D(_CameraReflectionTexture, coord+blur_coords[i]).rgb * _Adjust;
	        	}
	        	return finalColor;
	        }

...
//Toonシェーディング + スぺキュラ
half4 ToonBaseLighting(Light mainLight, half3 worldNormalVec, float4 baseColor, half3 shadowColor)
{
    float lDot = dot(mainLight.direction, worldNormalVec);
    float lSmootsh = smoothstep(0, _ShadowSmooth, lDot);
    
    half3 lLerp = lerp(shadowColor, baseColor.rgb, lSmootsh);
    half4 baseToonLightingColor = float4(lLerp, 1);
    ...
    float4 screenPos)
{
    ...
    {
        //ライティング計算(Toon)
        baseLightingColor = ToonBaseLighting(mainLight, inputData.baseInputData.normalWS, baseColor, ShadowColor) * INV_PI * _DiffuseIntensity;
        //スぺキュラ足す
...
#if ENABLE_EDGE_RIM        
        rimLightColor.rgb +=  _EnableEdgeRim * EdgeHighlight(screenPos, uv) * _EdgeRimColor;
#endif

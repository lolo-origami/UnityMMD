...
#include "LLToonLighting2.hlsl"
#include "LLToonOutline2.hlsl"
...
//#endif
    ...
    inputData.baseInputData.bakedGI = SAMPLE_GI(input.staticLightmapUV, input.dynamicLightmapUV, input.vertexSH, inputData.baseInputData.normalWS);
#else
...
half4 LLFragmentChara(Varyings input) : SV_Target
{
    ...
    // 計算した要素を足し合わせる
    half4 finalColor = lllData.BaseToonLightingColor; 
    ...
    finalColor.rgb = lerp(finalColor.rgb, shift(finalColor.rgb, half3(0.0, _OutlineSaturation, lerp(_OutlineBrightness, saturate(_OutlineBrightness * 2.0), lllData.RampOutline * _OutlineLightAffects))), outlineFactor);

...
half _VolumeIntensityButo[8]; // Intensity scalar (used for additive blend modes)
int _VolumeBlendMode[8]; // BlendMode per Volume
half _VolumeBlendFalloff[8]; // Blend Falloff per Volume
...
half GetFogDensityByNoise(Texture3D NoiseTexture, SamplerState Sampler, half3 RayPosition, half InvNoiseScale, half3 WindVelocity, half NoiseMin, half NoiseMax, int Octaves)
{
	...
	{
		value += amp * SAMPLE_TEXTURE3D_LOD(NoiseTexture, Sampler, uvw + wind, 0);
		c += amp;
		uvw *= _Lacunarity;
		amp *= _Gain;
	}
...
half GetFogFalloff(float3 RayPosition)
{
	...
	for (int i = 0; i < _VolumeCountButo; i++)
	{
		...
		half v = Remap(_VolumeDataButo[i].w * _VolumeBlendFalloff[i], _VolumeDataButo[i].w, remapVals.x, remapVals.y, d);
	...
	for (int i = 1; i <= SampleCount; i++)
	{
		...
		if (sampleExtinction > 0.0001)
		{
			...
			half finalShading = shadowAttenuation;
				...
				int stepCount = 4;
				for (int f = 1; f <= stepCount; f++)
				{
					float relativeStep = f / float(stepCount);
					half selfAttenStepLength = relativeStep * relativeStep * AttenuationBoundarySize;
					...
					shadowSelfAttenuation *= exp(-density * extinction * selfAttenStepLength * (1.0 - relativeStep));
				}

...
public class MMD_VmdCameraLoad : MonoBehaviour {
    ...
    private const int ARRAY_MAX = 100000000;
    ...
    public void CameraPosUpdate(float progressTime)
    {
        if (!_initialize || Cam_m == null)
        {
            Initialize();
        }
        //カメラ情報が完成したらカメラスタート
        ...
        nowframe = getAnimationFrame(progressTime);
    }

    private float getAnimationFrame(float progress)
    {
        ...
        if (clipInfoList != null)
        {
            ...
            returnValue = clip.length * progress * clip.frameRate;
        }
